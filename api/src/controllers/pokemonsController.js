...
const siluetas=[
  ...
  function getRandomInteger() {//funcion que retorna un numero del 0 al 9
...
const createPokemon = async (name, type, hp, attack, defense, speed, height, weight) => {
  //el método Pokemons.create me devuelve una promesa por eso le hago await
  const newPokemon = await Pokemons.create({name, image:imageUrl, type, hp, attack, defense, speed, height, weight}); 
  return newPokemon;
};
//-----------------------------------------------------------------------------------------------------------------------------
...
const getPokemonById = async (idPokemon, source) => {
  if(source === "api") {
    all = (await axios.get(`https://pokeapi.co/api/v2/pokemon/${idPokemon}`)).data
    // console.log(all);
...
const searchPokemonByName = async (name)=>{
  ...
  const allPromises = allUrls.map(getPokemonByIdUrl);//con el map ejecuto la función para cada url y allPromises será un array de promesas
  const allPokemons = await Promise.all(allPromises);// uso Promise.all para devolver todas las promesas resueltas en el arreglo allPokemons
  const filteredPokemons=allPokemons.filter((pokemon)=>pokemon.name.toLocaleLowerCase().includes(name.toLowerCase()));//bsucamos en el arreglo name
...
module.exports = { createPokemon, getPokemonById, searchPokemonByName, getFortyPokemons, getAllTypes };
